...
您也可以使用`JSX`和[`createRoutesFromElements`](https://reactrouter.com/en/main/utils/create-routes-from-elements)声明路由，元素的属性与路由对象的属性相同：
...
在本例中， `<h1>Layout</h1>` 将通过布局路由的 [Outlet](https://reactrouter.com/en/main/components/outlet) 与每个子路由的 `element` prop 一起呈现。

## `index`

确定路由是否为索引路由。索引路由通过父路由的 URL 呈现到父路由 [Outlet](https://reactrouter.com/en/main/route/outlet) 中（就像默认的子路由）。
...
这些特殊路线一开始可能会让人一头雾水，因此我们在这里专门为它们编写了一份指南：[索引路由](https://reactrouter.com/en/main/start/concepts#index-routes)。
...
路由加载器在路由渲染前被调用，并通过[`useLoaderData`](https://reactrouter.com/en/main/hooks/use-loader-data)为元素提供数据。
...
> IMPORTANT
>
> 如果您没有使用数据路由器（如[`createBrowserRouter`](https://reactrouter.com/en/main/routers/create-browser-router)），这将毫无用处

详情请查看 [loader](https://reactrouter.com/en/main/route/loader) 文档。

## `action`

当从[Form](https://reactrouter.com/en/main/components/form)、[fetcher](https://reactrouter.com/en/main/hooks/use-fetcher)或[submission](https://reactrouter.com/en/main/hooks/use-submit) 向路由发送提交信息时，路由操作将被调用。
...
更多详情，请参阅 [action](https://reactrouter.com/en/main/route/action) 文档。
...
更多详情，请参阅 [errorElement](https://reactrouter.com/en/main/route/error-element) 文档。

## `handle`

任何特定于应用程序的数据。有关详情和示例，请参阅 [useMatches](https://reactrouter.com/en/main/hooks/use-matches) 文档。
...
更多详情，请参阅 [lazy](https://reactrouter.com/en/main/route/lazy) 文档。