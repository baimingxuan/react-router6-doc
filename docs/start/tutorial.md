...
在本教程中，我们将使用[Vite](https://vitejs.dev/guide/)作为我们的捆绑器和开发服务器。您需要为命令行工具安装[Node.js。](https://nodejs.org/)`npm`
...
我们为本教程准备了一些预先编写的 CSS，因此我们可以专注于 React Router。随意严厉地判断它或编写你自己的 😀（我们做了我们通常不会在 CSS 中做的事情，以便本教程中的标记可以保持尽可能少。）

👉将**[此处找到](https://gist.githubusercontent.com/ryanflorence/ba20d473ef59e1965543fa013ae4163f/raw/499707f25a5690d490c7b3d54c65c65eb895930c/react-router-6.4-tutorial-css.css)****的教程 CSS 复制/粘贴到`src/index.css`**

本教程将创建、读取、搜索、更新和删除数据。一个典型的网络应用程序可能会与您的网络服务器上的 API 对话，但我们将使用浏览器存储并伪造一些网络延迟来保持这一点。这些代码都与 React Router 无关，所以只需复制/粘贴所有代码即可。

👉将**[此处找到](https://gist.githubusercontent.com/ryanflorence/1e7f5d3344c0db4a8394292c157cd305/raw/f7ff21e9ae7ffd55bfaaaf320e09c6a08a8a6611/contacts.js)****的教程数据模块复制/粘贴到`src/contacts.js`**

src 文件夹中所需的全部是`contacts.js`、`main.jsx`和`index.css`。您可以删除任何其他内容（例如`App.js`和`assets`等）。

👉**删除未使用的文件，`src/`所以你只剩下这些：**
...
如果您的应用程序正在运行，它可能会暂时崩溃，继续前进 😋。这样，我们就可以开始了！
